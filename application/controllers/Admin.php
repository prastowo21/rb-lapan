...
class Admin extends Pengguna {
	...
	public function __construct()
	{
		...
		if ($id_role != 1)
		{
			...
			$this->flashmsg('<i class="fa fa-warning"></i> Anda harus login sebagai admin', 'warning');
			redirect('login');
			exit;
		}
		...
		$this->data['admin'] = $this->admin_m->get_row(array('id_user' => $this->session->userdata('id_user')));
		...
		$file_ext = array('bmp', 'jpg', 'png');
				if (isset($temp_adm))
				{
					...
					{
						if (file_exists($img_filedir['pas_foto'] . $temp_adm->id_admin . '.' . $ext)){
							$this->data['foto'] = $temp_adm->id_admin . '.' . $ext;
						}
						else if (file_exists($img_filedir['pas_foto'] . $temp_adm->id_admin . '.' . strtoupper($ext))){
							$this->data['foto'] = $temp_adm->id_admin . '.' . strtoupper($ext);
						}
					}
				}
	}
	...
	public function profile(){
		...
		if (isset($id_user)){
			$data = $this->admin_m->get_row(array('id_user'=> $id_user));
		...
		if ($this->POST('submit')){
		 ...
		 //    $foto_baru = $this->up_foto_baru->data();
			...
			if($this->admin_m->update($this->data['admin']->id_admin, $data))
	...
	public function news_management(){
		...
		if($this->POST('tambah')){
			...
			if (!$this->berita_m->required_input($required))
			{
				...
				redirect('admin/news-management');
				exit;
			}
				...
				$data = array(
					'id_user'			=> $this->data['admin']->id_user,
					'tanggal_berita' 	=> $this->POST('tanggal_berita'),
					'judul_berita'		=> $this->POST('judul_berita'),
					'slug'				=> $slug,
					'deskripsi_berita'	=> $this->POST('deskripsi_berita'),
		...
		if($this->POST('edit')){
			...
			$slug = url_title($judul_berita_baru, '-', TRUE);
			...
			{	
				...
				$data = array(
					...
					'judul_berita'		=> $this->POST('judul_berita_baru'),
					// 'slug'				=> url_title($this->POST('judul_berita_baru'), '-', TRUE),
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/news_management';
		$this->template_admin($this->data);
	}

	// public function news_management_edit(){
	// 	$this->load->model('berita_m');

	// 	$id_berita = $this->uri->segment(3);

	// 	$this->data['id_berita'] = $id_berita;
	// 	$this->data['berita'] = $this->berita_m->get_row(['id_berita' => $id_berita]);
	...
	// 		if (!empty($id_berita))
	// 		{	
	// 			// $this->data['id_blok_kuliah'] = $this->kelas_kuliah_m->get_row(['id_kelas_kuliah' => $id_kelas_kuliah])->id_blok_kuliah;
	// 			$data = [
	// 				'id_berita'			=> $id_berita,
	// 				'id_user'			=> $this->data['admin']->id_user,
	// 				'tanggal_berita'	=> $this->POST('tanggal_berita'),
	...
	// 			$this->berita_m->update($this->POST('id_berita'),$data);
	// 			$this->flashmsg('<i class="fa fa-check"></i> Anda telah berhasil mengedit berita');
	// 			redirect('admin/news-management-edit/'.$id_berita);
	// 			exit;
	// 		}
	// 		else
	// 		{
	...
	// 	$this->data['title']	= $this->title;
	// 	$this->data['content']	= 'admin/news_management_edit';
	// 	$this->template_admin($this->data);
	// }
	...
	public function admin_management(){
		...
		$this->data['pengguna'] = $this->user_m->get(array('id_role' => '1'));

		if (isset($id_user)) 
		{
			$data = $this->user_m->get_row(array('id_user'=>$id_user));
			echo json_encode($data);
			exit;
		}
		...
		if ($this->POST('tambah')){
			...
			$random_userid = 'adm_' . $this->_generateRandomString(12);
			$options = array('cost' => 10);

			if($this->user_m->get_row(array('id_user' => $id_user)) > 0){
				$this->flashmsg('<i class="fa fa-warning"></i> Username sudah ada sebelumnya', 'danger');
				redirect('admin/admin-management');
				exit;
			}

			while($this->admin_m->idExists($random_userid)){
				$random_userid = 'adm_' . $this->_generateRandomString(12);
			}
			...
			if (!empty($id_user) && !empty($password))
			{
				...
				if ($this->POST('add_news') == "on"){
					$add_news = "on";
				}else{
					$add_news = "off";
				}
				if ($this->POST('add_user') == "on"){
					$add_user = "on";
				}else{
					$add_user = "off";
				}
				if ($this->POST('add_banner') == "on"){
					$add_banner = "on";
				}else{
					$add_banner = "off";
				}
				if ($this->POST('add_menu') == "on"){
					$add_menu = "on";
				}else{
					$add_menu = "off";
				}
				if ($this->POST('add_diagram') == "on"){
					$add_diagram = "on";
				}else{
					$add_diagram = "off";
				}
				if ($this->POST('add_capaian') == "on"){
					$add_capaian = "on";
				}else{
					$add_capaian = "off";
				}
				if ($this->POST('add_ikm') == "on"){
					$add_ikm = "on";
				}else{
					$add_ikm = "off";
				}
				if ($this->POST('add_ipk') == "on"){
					$add_ipk = "on";
				}else{
					$add_ipk = "off";
				}
				if ($this->POST('add_pub_news') == "on"){
					$add_pub_news = "on";
				}else{
					$add_pub_news = "off";
				}
				if ($this->POST('add_pub_banner') == "on"){
					$add_pub_banner = "on";
				}else{
					$add_pub_banner = "off";
				}
				...
				$data_privilege = array(
					...
					'ikm'			=> $add_ikm,
					'ipk'			=> $add_ipk,
					'pub_news'		=> $add_pub_news,
					'pub_banner'	=> $add_pub_banner
				);

				//Upload config pasfoto
			    $config = array();
			    $config['file_name'] = $random_userid;
			    $config['upload_path'] = $this->base_filedir . "/pas_foto/";

			    $config['allowed_types'] = 'jpeg|jpg|png|bmp';
			    $config['max_size'] = '100000';
			    $this->load->library('upload', $config, 'up_pasfoto'); //Custom pasfoto upload config.

			    if(!$this->up_pasfoto->do_upload('pasfoto'))
			    {
			    	//show error messages
			    	$this->flashmsg("Error on pas foto file:" . $this->up_pasfoto->display_errors(),'danger');
					redirect('admin/admin-management');
					exit;
			    }

			    //Store pas foto data array on a variable
			    $pasfoto_data = $this->up_pasfoto->data();
				
				$this->user_m->insert($data_user);
				$this->admin_m->insert($data_admin); 
				$this->privilege_m->insert($data_privilege);
				$this->flashmsg('<i class="fa fa-check"></i> Anda telah berhasil menambah admin');
				redirect('admin/admin-management');
				exit;
			}
			else
			{
				$this->flashmsg('<i class="fa fa-warning"></i> Gagal menambah admin', 'danger');
				redirect('admin/admin-management');
				exit;
			}
		}
		...
		if ($this->POST('change')){
			...
			elseif (password_verify($old_password_input, $old_password))
			{
				$data = array(
					'password'		=> password_hash($this->POST('new_password'), PASSWORD_DEFAULT, $options)
				);
		...
		if ($this->POST('changePrivilege')){
			if ($this->POST('chg_news') == "on"){
				$chg_news = "on";
			}else{
				$chg_news = "off";
			}
			if ($this->POST('chg_user') == "on"){
				$chg_user = "on";
			}else{
				$chg_user = "off";
			}
			if ($this->POST('chg_banner') == "on"){
				$chg_banner = "on";
			}else{
				$chg_banner = "off";
			}
			if ($this->POST('chg_menu') == "on"){
				$chg_menu = "on";
			}else{
				$chg_menu = "off";
			}
			if ($this->POST('chg_diagram') == "on"){
				$chg_diagram = "on";
			}else{
				$chg_diagram = "off";
			}
			if ($this->POST('chg_capaian') == "on"){
				$chg_capaian = "on";
			}else{
				$chg_capaian = "off";
			}
			if ($this->POST('chg_ikm') == "on"){
				$chg_ikm = "on";
			}else{
				$chg_ikm = "off";
			}
			if ($this->POST('chg_ipk') == "on"){
				$chg_ipk = "on";
			}else{
				$chg_ipk = "off";
			}
			if ($this->POST('chg_pub_news') == "on"){
				$chg_pub_news = "on";
			}else{
				$chg_pub_news = "off";
			}
			if ($this->POST('chg_pub_banner') == "on"){
				$chg_pub_banner = "on";
			}else{
				$chg_pub_banner = "off";
			}
			...
			$data = array(
				'id_user'		=> $this->POST('id_user_privilege'),
				'news'			=> $chg_news,
				'user'			=> $chg_user,
				'banner'		=> $chg_banner,
				'menu'			=> $chg_menu,
				...
				'ikm'			=> $chg_ikm,
				'ipk'			=> $chg_ipk,
				'pub_news'		=> $chg_pub_news,
				'pub_banner'	=> $chg_pub_banner
			);

			if ($this->privilege_m->update($this->POST('id_user_privilege'), $data))
				$this->flashmsg('<i class="fa fa-check"></i> Anda telah berhasil mengubah privilege');
			else
				$this->flashmsg('<i class="fa fa-warning"></i> Gagal mengubah privilege', 'danger');
				redirect('admin/admin-management');
				exit;
		}
		...
		if ($this->POST('delete')){
				...
				if ($this->user_m->delete($this->POST('id_user_del')))
				$this->flashmsg('<i class="fa fa-check"></i> Berhasil menghapus admin');
				else $this->flashmsg('<i class="fa fa-warning"></i> Gagal menghapus admin', 'danger');
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/admin_management';
		$this->template_admin($this->data);
	}

	public function editor_management(){
		$this->load->model('user_m');
		$this->load->model('editor_m');
		$this->load->model('privilege_m');
		...
		$this->data['pengguna'] = $this->user_m->get(array('id_role' => '2'));

		if (isset($id_user)) 
		{	
			// $data = $this->user_m->get_row(array('id_user'=>$id_user));
			$data = $this->privilege_m->get_row(array('id_user' => $id_user));
			echo json_encode($data);
			exit;
		}
		...
		if ($this->POST('tambah')){
			...
			$id_role = $this->POST('id_role');
			$password = $this->POST('password');
			$random_userid = 'edt_' . $this->_generateRandomString(12);
			$options = array('cost' => 10);
			...
			while($this->editor_m->idExists($random_userid)){
				$random_userid = 'edt_' . $this->_generateRandomString(12);
			}
			...
			if (!empty($id_user) && !empty($password))
			{
			    ...
			    if(!$this->up_pasfoto->do_upload('pasfoto'))
			    {
			    	//show error messages
			    	$this->flashmsg("Error on pas foto file:" . $this->up_pasfoto->display_errors(),'danger');
					redirect('admin/editor-management');
					exit;
			    }
				...
				$this->user_m->insert($data_user);
				$this->editor_m->insert($data_editor);
				$this->privilege_m->insert($data_privilege); 
				$this->flashmsg('<i class="fa fa-check"></i> Anda telah berhasil menambah user');
				redirect('admin/editor-management');
				exit;
			}
			else
			{
				$this->flashmsg('<i class="fa fa-warning"></i> Gagal menambah user', 'danger');
				redirect('admin/editor-management');
				exit;
			}
		}
		...
		if ($this->POST('delete')){
				...
				if ($this->user_m->delete($this->POST('id_user_del')))
					$this->flashmsg('<i class="fa fa-check"></i> Berhasil menghapus user');
				else $this->flashmsg('<i class="fa fa-warning"></i> Gagal menghapus user', 'danger');
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/editor_management';
		$this->template_admin($this->data);
	}
	...
	public function banner_management(){
		...
		if ($this->POST('delete')){
				...
				if ($this->banner_m->delete($this->POST('id_banner_del')))
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/banner_management';
		$this->template_admin($this->data);
	}
	...
	public function vimi_management(){
		...
		if($this->POST('tambah')){
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/vimi-management');
				exit;
			}
		...
		if ($this->POST('delete')){
				...
				if ($this->menu_vimi_m->delete($this->POST('id_vimi_del')))
					...
					$this->flashmsg('<i class="fa fa-warning"></i> Gagal menghapus berita', 'danger');
					redirect('admin/vimi-management');
					exit;
		}

		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/vimi_management';
		$this->template_admin($this->data);
	}
	...
	public function tujuan_management(){
		...
		if($this->POST('tambah')){
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/tujuan-management');
				exit;
			}
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/tujuan_management';
		$this->template_admin($this->data);
	}
	...
	public function sasaran_management(){
		...
		if($this->POST('tambah')){
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/sasaran-management');
				exit;
			}
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/sasaran_management';
		$this->template_admin($this->data);
	}
	...
	public function peraturanrb_management(){
		...
		if($this->POST('tambah')){
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/peraturanrb-management');
				exit;
			}
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/peraturan_rb_management';
		$this->template_admin($this->data);
	}
	...
	public function quickwin_management(){
		...
		if($this->POST('tambah')){
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/quickwin-management');
				exit;
			}
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/quickwin_management';
		$this->template_admin($this->data);
	}
	...
	public function kebijakan_management(){
		...
		if($this->POST('tambah')){
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/kebijakan-management');
				exit;
			}
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/kebijakan_management';
		$this->template_admin($this->data);
	}
	...
	public function roadmap_management(){
		...
		if($this->POST('tambah')){
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/roadmap-management');
				exit;
			}
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/roadmap_management';
		$this->template_admin($this->data);	
	}
	...
	public function diagram_management(){
		...
		if($this->POST('tambah'))
		{
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/diagram-management');
				exit;
			}
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/diagram_management';
		$this->template_admin($this->data);
	}
	...
	public function manajemen_perubahan_management(){
		...
		if($this->POST('tambah')){
		    ...
		    if(!$this->up_mp_file->do_upload('dokumen'))
		    {
				...
				redirect('admin/manajemen-perubahan-management');
				exit;
		    }
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/manajemen_perubahan_management';
		$this->template_admin($this->data);
	}
	...
	public function penataan_peraturan_uu_management(){
		...
		if($this->POST('tambah')){
		    ...
		    if(!$this->up_ppuu_file->do_upload('dokumen'))
		    {
				...
				redirect('admin/penataan-peraturan-uu-management');
				exit;
		    }
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/penataan_peraturan_uu_management';
		$this->template_admin($this->data);
	}
	...
	public function penataan_dan_penguatan_organisasi_management(){
		...
		if($this->POST('tambah')){
		    ...
		    if(!$this->up_ppo_file->do_upload('dokumen'))
		    {
				...
				redirect('admin/penataan-dan-penguatan-organisasi-management');
				exit;
		    }
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/penataan_dan_penguatan_organisasi_management';
		$this->template_admin($this->data);
	}
	...
	public function penataan_tatalaksana_management(){
		...
		if($this->POST('tambah')){
		    ...
		    if(!$this->up_pt_file->do_upload('dokumen'))
		    {
				...
				redirect('admin/penataan-tatalaksana-management');
				exit;
		    }
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/penataan_tatalaksana_management';
		$this->template_admin($this->data);
	}
	...
	public function penataan_sistem_manajemen_sdm_management(){
		...
		if($this->POST('tambah')){
		    ...
		    if(!$this->up_psms_file->do_upload('dokumen'))
		    {
				...
				redirect('admin/penataan-sistem-manajemen-sdm-management');
				exit;
		    }
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/penataan_sistem_manajemen_sdm_management';
		$this->template_admin($this->data);
	}
	...
	public function penguatan_pengawasan_management(){
		...
		if($this->POST('tambah')){
		    ...
		    if(!$this->up_pp_file->do_upload('dokumen'))
		    {
				...
				redirect('admin/penguatan-pengawasan-management');
				exit;
		    }
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/penguatan_pengawasan_management';
		$this->template_admin($this->data);
	}
	...
	public function penguatan_akuntabilitas_kinerja_management(){
		...
		if($this->POST('tambah')){
		    ...
		    if(!$this->up_pak_file->do_upload('dokumen'))
		    {
				...
				redirect('admin/penguatan-akuntabilitas-kinerja-management');
				exit;
		    }
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/penguatan_akuntabilitas_kinerja_management';
		$this->template_admin($this->data);
	}
	...
	public function peningkatan_kualitas_pelayanan_publik_management(){
		...
		if($this->POST('tambah')){
		    ...
		    if(!$this->up_pkpb_file->do_upload('dokumen'))
		    {
				...
				redirect('admin/peningkatan-kualitas-pelayanan-publik-management');
				exit;
		    }
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/peningkatan_kualitas_pelayanan_publik_management';
		$this->template_admin($this->data);
	}
	...
	public function buletin_management(){
		...
		if ($this->POST('tambah')){
			...
			{
				$this->flashmsg("<i class='fa fa-warning'></i> Penambahan data gagal, anda harus mengisi data dengan lengkap",'warning');
				redirect('admin/buletin-management');
				exit;
			}
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/buletin_management';
		$this->template_admin($this->data);
	}
	...
	public function ikm_management()
	{
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/ikm_management';
		$this->template_admin($this->data);
	}
	...
	public function ipk_management()
	{
		...
		$this->data['title']	= $this->title;
		$this->data['content']	= 'admin/ipk_management';
		$this->template_admin($this->data);
	}
