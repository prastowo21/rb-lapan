...
<link href="<?= base_url('assets/gentelella-master') ?>/vendors/bootstrap-imageupload/css/fileinput.min.css" rel="stylesheet">
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                  ...
                  <div class="x_content">
                    ...
                    <div class="col-md-12 col-sm-12 col-xs-12">
                      <div class="x_panel">
                        <div class="x_title">
                          <h2>Capaian Management <small>Penguatan Pengawasan</small></h2>
                          ...
                          <table id="datatable" class="table table-striped table-bordered">
                            ...
                            <?php foreach ($PP as $pp): ?>
                              ...
                              <tr>
                                <td><?= $No++; ?></td>
                                <td><?= $pp->title ?></td>
                                <td><a href="<?= base_url() ?>doc_file/capaian/Penataan Sistem Manajemen SDM/<?= $pp->isi ?>" target="_blank"><?= $pp->isi ?></a></td>
                                <td>
                                  <?php $id_role = $this->user_m->get_row(array('id_user' => $pp->id_user))->id_role ; ?>
                                  <?php if ($id_role == 1): ?>
                                    Diunggah oleh <?= $this->admin_m->get_row(array('id_user' => $pp->id_user))->nama ?> (Admin)
                                  <?php elseif ($id_role == 2): ?>
                                    Diunggah oleh <?= $this->editor_m->get_row(array('id_user' => $pp->id_user))->nama ?> (Editor)
                                  <?php endif; ?>
                                </td>
                                <td><?= $pp->tanggal ?></td>
                                <td>
                                  <a href="#edit" onclick="retrieveData('<?= $pp->id_penguatan_pengawasan ?>');" data-toggle="modal" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>
                                  <a href="#delete" onclick="deleteData('<?= $pp->id_penguatan_pengawasan ?>');" data-toggle="modal" class="btn btn-danger btn-xs"><i class="fa fa-trash-o" ></i> Delete </a>
                                </td>
                              </tr>
                    ...
                    <?= form_open('editor/penguatan-pengawasan-management') ?>
                      <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                          ...
                          <div class="modal-footer">
                            <input type="hidden" name="id_penguatan_pengawasan_del" id="id_penguatan_pengawasan_del" value="">
                    ...
                    <?= form_open_multipart('editor/penguatan-pengawasan-management',array('class' => 'form-horizontal')) ?>
                          ...
                          <div class="modal-body">
                            <div class="form-group">
                              ...
                              <div class="col-md-6 col-sm-6 col-xs-12">
                                ...
                                <input type="hidden" id="id_penguatan_pengawasan_baru" name="id_penguatan_pengawasan_baru" required="required" class="form-control col-md-7 col-xs-12">
                              </div>
                            </div>
                            <div class="form-group">
                              <label class="control-label col-md-3 col-sm-3 col-xs-12" for="last-name">Dokumen <span class="required">*</span>
    ...
    function retrieveData(id_penguatan_pengawasan) {
        $.get('<?= base_url('index.php/editor/penguatan-pengawasan-management') ?>' + '/' + id_penguatan_pengawasan, function(data) {
          data = JSON.parse(data);
          $('#id_penguatan_pengawasan_baru').val(data.id_penguatan_pengawasan);
          $('#title_baru').val(data.title);
          tinymce.EditorManager.get('dokumen_baru').setContent(data.isi);
        });
      }

      function deleteData(id_penguatan_pengawasan){
          $('#id_penguatan_pengawasan_del').val(id_penguatan_pengawasan); 
      }
